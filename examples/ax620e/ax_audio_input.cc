...
static void *AoCtrlThread(void *arg)
{
    ...
    while (!gLoopExit) {
        ...
        case 'r':
        case 'R': {
            AX_AUDIO_SAMPLE_RATE_E enInSampleRate = (AX_AUDIO_SAMPLE_RATE_E)gResRate;
...
static int AudioInput()
{
    ...
    // memset(&stPoolConfig, 0, sizeof(AX_POOL_CONFIG_T));//结构体实参使用前建议先清 0
    stPoolConfig.MetaSize = 8192;
    stPoolConfig.BlkSize = 7680;
    stPoolConfig.BlkCnt = 33;
    stPoolConfig.IsMergeMode = AX_FALSE;//AX_FALSE;
    stPoolConfig.CacheMode = AX_POOL_CACHE_MODE_NONCACHE;//AX_POOL_CACHE_MODE_NONCACHE;
...
static int AudioOutput()
{
    ...
    stAttr.u32PeriodSize = gPeriodSize;
    stAttr.u32PeriodCount = gPeriodCount;
    stAttr.bInsertSilence = (AX_BOOL)gInsertSilence;
    ...
    do {
        ...
        stFrmInfo.u64VirAddr = (AX_U8*)AX_POOL_GetBlockVirAddr(stFrmInfo.u32BlkId);
        num_read = fread(stFrmInfo.u64VirAddr, 1, BlkSize, input_file);
        if (readMixFrm) {
            stMixFrmInfo.u32BlkId = AX_POOL_GetBlock(PoolId, BlkSize, NULL);
            stMixFrmInfo.u64VirAddr = (AX_U8*)AX_POOL_GetBlockVirAddr(stMixFrmInfo.u32BlkId);
